    ...
    def process_item(self, item, spider):
      ...
      if(es.exists(index="gearbest_index",doc_type='product_type',id=item['code'])):
        ...
        for lf in list_of_fieds:
          if lf in item and len(item[lf])>0 and lf in get_doc['_source']:
            last_item=item[lf][0]
            #item[lf]=[]
            try:
              if len(get_doc['_source'][lf])>0:
                get_doc['_source'][lf].append(last_item)
                item[lf]=get_doc['_source'][lf]
                print '(OK_OK_OK_OK_OK_OK_OK_OK_OK_OK)='+item['code']+' '+lf
            except TypeError:
